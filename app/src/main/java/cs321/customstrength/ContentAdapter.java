      ...
      itemView.setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View v) {
          ...
          if (LoadExerciseData.PRELOADED_EXERCISES.containsKey(item) == true) {
            AllExercises.delete = false;
            intent = new Intent(v.getContext(), ExerciseInfo.class);
            intent.putExtra("Preloaded_Exercise", item);
          }
          else if(AllExercises.delete) {
            AllExercises.delete = false;
            LoadExerciseData.removeCustomExercise(item);
            intent = new Intent(v.getContext(), AllExercises.class);
            ((Activity)v.getContext()).finish();
            Toast.makeText(v.getContext(), item + " has been permanently removed", Toast.LENGTH_SHORT).show();
          }
          else{
